<header
    class="fixed top-0 right-0 left-64 h-16 bg-brand-surface/60 dark:bg-slate-900/80 backdrop-blur-xl z-40 flex justify-between items-center px-8 border-b border-brand-teal/10 dark:border-brand-teal/20 transition-all">
    <!-- Search Bar -->
    <form action="{{ route('member.search') }}" method="GET" class="flex items-center gap-6">
        <div class="relative group">
            ...
            <input name="q" value="{{ request('q') }}"
                class="pl-10 pr-4 py-2 bg-white/50 border border-brand-teal/10 dark:border-brand-teal/20 rounded-full text-sm w-72 focus:ring-2 focus:ring-brand-teal/20 focus:bg-white transition-all outline-none placeholder:text-brand-slate/50 dark:placeholder:text-slate-300 font-light"
    ...
    <div class="flex items-center gap-3">
        ...
        <div class="relative group">
            <div class="flex items-center gap-3 pl-2 cursor-pointer">
                ...
                <img alt="User Avatar"
                    class="w-8 h-8 rounded-full object-cover border border-brand-teal/20 dark:border-brand-teal/30 group-hover:border-brand-teal/50 transition-all"
                    src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=F5FAFB&color=282B2A&bold=true" />
            </div>

            <!-- Dropdown Box -->
            <div
                class="absolute right-0 mt-3 w-48 bg-white dark:bg-slate-800 border border-brand-teal/10 dark:border-brand-teal/20 rounded-2xl shadow-[0_10px_40px_-10px_rgba(48,71,78,0.15)] opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50 overflow-hidden translate-y-2 group-hover:translate-y-0">
                <a href="/"
                    class="block px-4 py-3 text-sm font-light text-brand-slate dark:text-slate-300 hover:bg-brand-surface dark:hover:bg-slate-700 hover:text-brand-dark dark:hover:text-white transition-colors">
                ...
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit"
                        class="w-full text-left block px-4 py-3 text-sm font-medium text-red-500 hover:bg-red-50 dark:hover:bg-red-900/50 hover:text-red-600 dark:hover:text-red-400 transition-colors">
                        <span class="material-symbols-outlined text-[16px] align-middle mr-2"
