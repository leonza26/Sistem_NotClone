    ...
    <div class="px-10 pb-12">
        <header class="max-w-3xl mb-8">
            ...
            <h2 class="font-headline text-display-md text-4xl font-extrabold text-on-surface leading-tight tracking-tight">
                Edit <span class="text-primary">Project.</span>
            </h2>
        </header>
                ...
                <form action="{{ route('member.projects.update', $project) }}" method="POST">
                    ...
                    <div class="mb-8">
                        ...
                        <textarea name="description" id="description" rows="4"
                            class="block w-full rounded-xl border-outline-variant bg-surface focus:border-primary focus:ring-primary sm:text-sm px-4 py-3">{{ old('description', $project->description) }}</textarea>
                        @error('description')
                            <p class="text-error text-xs mt-2 font-semibold">{{ $message }}</p>
                        @enderror
                    </div>
