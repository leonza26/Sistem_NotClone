            ...
            <form action="{{ route('member.projects.update', $project) }}" method="POST">
                ...
                <div class="space-y-1.5 mb-8">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="w-full px-4 py-3.5 bg-brand-surface dark:bg-slate-900 border border-brand-teal/20 dark:border-brand-teal/30 rounded-xl focus:ring-2 focus:ring-brand-orange/20 focus:border-brand-orange transition-all text-sm outline-none text-brand-dark dark:text-white font-light leading-relaxed custom-scrollbar">{{ old('description', $project->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-[11px] mt-1.5 font-medium ml-1">{{ $message }}</p>
                    @enderror
                </div>
