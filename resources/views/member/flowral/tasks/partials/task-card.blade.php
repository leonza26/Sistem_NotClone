<div data-id="{{ $task->id }}" class="bg-surface-container-lowest p-5 rounded-2xl editorial-shadow group border border-transparent hover:border-primary/20 transition-all relative cursor-grab active:cursor-grabbing">
    <div class="flex justify-between items-start mb-4">
        ...
        <div x-data="{ openMenu: false }" class="relative z-10">
            <button @click="openMenu = !openMenu" @click.away="openMenu = false" class="opacity-0 group-hover:opacity-100 transition-opacity text-slate-400 hover:text-primary">
            ...
            <div x-show="openMenu" x-cloak 
                ...
                <a href="{{ route('member.tasks.edit', $task) }}" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 hover:text-primary font-medium">Edit</a>
                <button type="button" @click="$dispatch('open-delete-modal', { url: '{{ route('member.tasks.destroy', $task) }}', message: 'Yakin hapus task \'{{ $task->title }}\'?' })" 
    ...
    <div class="flex justify-between items-center pt-4 border-t border-surface-container">
        ...
        <div class="flex items-center gap-2">
            ...
            @else
                <div class="w-6 h-6 rounded-full bg-slate-100 border border-slate-200 flex items-center justify-center">
                    <span class="material-symbols-outlined text-[12px] text-slate-400" data-icon="person_off">person_off</span>
                </div>
        ...
        <!-- Due Date -->
        <div class="flex items-center gap-1 {{ $task->due_date && \Carbon\Carbon::parse($task->due_date)->isPast() ? 'text-red-500' : 'text-slate-400' }}">
