    ...
    <div class="flex flex-col h-[calc(100vh-4rem)] max-w-5xl mx-auto" x-data="copilotChat()">
        ...
        <header class="flex items-center justify-between px-8 py-6 border-b border-brand-teal/5">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="font-outfit text-xl font-medium text-brand-dark dark:text-white leading-tight">Flowral Copilot</h2>
            ...
            <button @click="clearChat()" x-show="messages.length > 0"
                class="px-4 py-2 text-[12px] font-medium bg-brand-surface dark:bg-slate-900 text-brand-slate dark:text-slate-300 hover:text-brand-dark dark:hover:text-white hover:bg-brand-teal/10 rounded-full transition-colors border border-brand-teal/10 dark:border-brand-teal/20">
                Clear Chat
            </button>
        </header>
        ...
        <div class="flex-1 overflow-y-auto px-8 pb-10 custom-scrollbar flex flex-col pt-6" id="chat-container">
            ...
            <div x-show="messages.length === 0"
                ...
                <div
                    class="w-20 h-20 bg-brand-surface dark:bg-slate-900 rounded-[32px] border border-brand-teal/10 dark:border-brand-teal/20 flex items-center justify-center mb-6 shadow-sm dark:shadow-none">
                    <span class="material-symbols-outlined text-[32px] text-brand-orange">auto_awesome</span>
                </div>
                <h3 class="font-outfit text-3xl font-medium text-brand-dark dark:text-white mb-3">How can I help you today?</h3>
                <p class="text-brand-slate dark:text-slate-300 font-light text-sm max-w-md mx-auto">
                ...
                <div class="flex flex-wrap justify-center gap-3 mt-10">
                    <button
                        ...
                        class="px-5 py-3 bg-white dark:bg-slate-800 border border-brand-teal/10 dark:border-brand-teal/20 rounded-2xl text-[13px] font-light text-brand-dark dark:text-white hover:border-brand-orange hover:shadow-md transition-all text-left flex items-center gap-3">
            ...
            <template x-for="(msg, index) in messages" :key="index">
                ...
                <div class="flex gap-4 mb-6" :class="msg.role === 'user' ? 'flex-row-reverse' : ''">
                    ...
                    <div x-show="msg.role === 'user'"
                        class="w-8 h-8 rounded-full bg-brand-surface dark:bg-slate-900 border border-brand-teal/10 dark:border-brand-teal/20 flex-shrink-0 mt-1">
                    ...
                    <div :class="msg.role === 'user' ? 'bg-brand-surface dark:bg-slate-700 rounded-[24px] rounded-tr-sm p-4 text-[14px] text-brand-dark dark:text-white font-light border border-brand-teal/10 dark:border-brand-teal/20 max-w-[80%]' : 'bg-white dark:bg-slate-800 rounded-[24px] rounded-tl-sm p-5 text-[14px] text-brand-dark dark:text-white font-light border border-brand-teal/10 dark:border-brand-teal/20 shadow-sm dark:shadow-none max-w-[80%] leading-relaxed'"
                        x-html="msg.text">
                    </div>
            ...
            <div x-show="isLoading" class="flex gap-4 mb-6 animate-pulse">
                ...
                <div
                    class="bg-white dark:bg-slate-800 rounded-[24px] rounded-tl-sm p-5 w-48 border border-brand-teal/10 dark:border-brand-teal/20 shadow-sm dark:shadow-none flex flex-col gap-2">
        ...
        <!-- Input Area -->
        <div class="px-8 pb-8 pt-4 bg-gradient-to-t from-brand-surface dark:from-slate-900 to-transparent">
            <div
                class="relative bg-white dark:bg-slate-800 rounded-[32px] border border-brand-teal/20 dark:border-brand-teal/30 shadow-[0_10px_40px_-10px_rgba(48,71,78,0.1)] p-2 flex items-end transition-all focus-within:border-brand-orange focus-within:shadow-[0_10px_40px_-10px_rgba(229,117,0,0.15)]">

                <textarea x-model="userInput" @keydown.enter.prevent="sendMessage()" rows="1"
                    placeholder="Ask Copilot anything..."
                    class="w-full bg-transparent border-none focus:ring-0 resize-none max-h-32 px-3 py-3 text-[14px] font-light text-brand-dark dark:text-white placeholder:text-brand-slate/50 dark:placeholder:text-slate-300 custom-scrollbar"
            ...
            <p class="text-center text-[10px] text-brand-slate dark:text-slate-300 mt-4 font-light">AI can make mistakes. Consider verifying
